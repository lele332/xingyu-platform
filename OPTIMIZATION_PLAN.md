...
## 第六轮（2026-09-09）：AI 记忆可审计与技能上下文化

- AI 助手页新增「AI 记忆与反馈」：
  - 展示本机偏好条数、累计反馈、最近 8 条反馈。
  - 每条记忆可单独删除，支持一键清空。
  - 学习数据不会被删除，避免误操作。
- AI 技能调用全面接入上下文引擎：
  - `/学习规划`、`/智能排序`、`/知识卡片`、`/笔记整理` 现在都会参考用户画像、学习规律、状态、偏好和相关笔记。
  - 保留技能模式的结构化输出约束，避免上下文增强破坏原有任务格式。
- 修正记忆提取重复问题：一句显式“请记住……”只沉淀一条记忆，避免提示词噪音。
- Service Worker 缓存版本升级为 `xingyu-static-20260909-04`。

## 本轮验证
- `npm run check`：SRS、AI 记忆/上下文预算、11 项平台测试通过。
- `npm run test:pomo`：番茄钟暂停/恢复/重置通过。
- `npm run test:ui`：三视口布局、AI 洞察、AI 记忆学习/展示/删除、AI 反馈、保存笔记、主题与图标通过。


## 第七轮（2026-09-09）：大数据量笔记检索提速

- BM25 检索加入本机索引缓存：
  - 笔记全文、标题、标签只构建一次。
  - `Store.onSave` 后自动失效重建，避免读到旧笔记。
  - 查询从“两次动态正则扫描”改为“一次扫描 + split 计词频”，降低大内容量时的 CPU 开销。
- 单元测试扩展到 3000 篇笔记，验证相关笔记仍能排到第一位。
- Script/Service Worker 缓存版本更新，确保客户端拿到新上下文引擎。

## 验证
- `npm run check`：SRS、3000 笔记 BM25 检索、AI 上下文预算、11 项平台测试通过。
- `npm run test:ui`：三视口布局与 AI 记忆/反馈闭环通过。

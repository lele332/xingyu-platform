...
- **Sidebar intro** (`sidebarIntro`): on load the brand fades in, then group labels and the 8 nav items slide up (10px) in a staggered cascade (40ms apart). After the cascade the moving pill initializes at the active item.
- **Moving pill** (`initNavPill` / `navPillTo`): a `.nav-pill` background block slides smoothly to the active nav item on every view switch (450ms `power3.inOut`, animates `top/left/width/height` via `offsetTop/offsetLeft` so it never drifts during the intro animation). Initial placement uses `gsap.set`; view switches use `gsap.to`. This is the mature "moving highlight" pattern (iOS settings, VS Code sidebar).
- **Sidebar hover** (`initNav`): on hover the `.nav-label` eases 4px right (250ms `power2.out`), reversible on leave. (Originally included icon pop + a sweeping shine, but the nav items don't actually carry icons, so the text-shift feedback is what reads as "alive".)
- **Card tilt** (`initTilt`, desktop hover only): cards rotate toward the cursor via `gsap.quickTo` (`rotationX/Y`, ±5°, `transformPerspective: 900`) with a subtle scale bump on hover. Tweens are stored in a `WeakMap` and listeners bind once per card; only enabled when `(hover: hover) and (pointer: fine)`.

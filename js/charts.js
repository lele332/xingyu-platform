...
const Charts = (() => {
  ...
  function bars(container, opts) {
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      const val = max * (4 - i) / 4;
      svg += `<line x1="${pad.l}" y1="${y}" x2="${w - pad.r}" y2="${y}" stroke="rgba(26,24,20,0.08)"/>`;
      svg += `<text x="${pad.l - 6}" y="${y + 4}" text-anchor="end" font-size="10" fill="rgba(140,126,106,0.7)">${Math.round(val)}${unit}</text>`;
    }
    values.forEach((v, i) => {
      ...
      svg += `<text x="${x + bw / 2}" y="${y - 6}" text-anchor="middle" font-size="10" fill="rgba(203,213,225,0.62)">${v}</text>`;
      svg += `<text x="${x + bw / 2}" y="${h - 8}" text-anchor="middle" font-size="10" fill="rgba(203,213,225,0.38)">${labels[i]}</text>`;
    });
    svg += `</svg>`;

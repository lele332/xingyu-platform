...
const Charts = (() => {
  ...
  function donut(container, opts) {
    ...
    svg += `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="var(--fill-2)" stroke-width="${thickness}"/>`;
    ...
    svg += `<text x="${cx}" y="${cy - 4}" text-anchor="middle" font-size="20" font-weight="700" fill="var(--ink)" font-family="Georgia, serif">${centerLabel}</text>`;
    if (centerSub) svg += `<text x="${cx}" y="${cy + 18}" text-anchor="middle" font-size="11" fill="var(--ink-3)" font-family="-apple-system, sans-serif">${centerSub}</text>`;
    svg += `</svg>`;
  ...
  function bars(container, opts) {
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      const val = max * (4 - i) / 4;
      svg += `<line x1="${pad.l}" y1="${y}" x2="${w - pad.r}" y2="${y}" stroke="var(--rule-thin)"/>`;
      svg += `<text x="${pad.l - 6}" y="${y + 4}" text-anchor="end" font-size="10" fill="var(--ink-3)">${Math.round(val)}${unit}</text>`;
    }
    values.forEach((v, i) => {
      ...
      svg += `<text x="${x + bw / 2}" y="${y - 6}" text-anchor="middle" font-size="10" fill="var(--ink-2)">${v}</text>`;
      svg += `<text x="${x + bw / 2}" y="${h - 8}" text-anchor="middle" font-size="10" fill="var(--ink-3)">${labels[i]}</text>`;
    });
    svg += `</svg>`;
  ...
  function line(container, opts) {
    ...
    values.forEach((v, i) => {
      const [x, y] = pt(i, v);
      svg += `<circle cx="${x}" cy="${y}" r="3.5" fill="${color}" stroke="var(--paper-card)" stroke-width="2"/>`;
      svg += `<text x="${x}" y="${y - 10}" text-anchor="middle" font-size="10" fill="var(--ink-3)">${v}</text>`;
      svg += `<text x="${x}" y="${h - 8}" text-anchor="middle" font-size="10" fill="var(--ink-3)">${labels[i]}</text>`;
    });
    svg += `</svg>`;
    container.innerHTML = svg;
  }
